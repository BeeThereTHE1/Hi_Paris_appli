...
const isDraft = isSavedDraft || (isAuthor && !allOfficial.some(e => String(e.id) === String(exo.id))) || ((exo === null || exo === void 0 ? void 0 : exo.id) === 'preview');
...
async function validateExercise() {
    ...
    if (!list.find(e => e.id == exoId)) {
        list.push(Object.assign(Object.assign({}, exo), { date: new Date().toLocaleDateString('fr-FR') }));
        localStorage.setItem(key, JSON.stringify(list));
    }
    
    // NOUVEAU : Enregistrement en base de données Supabase
    if (user && user.email) {
        try {
