  ...
  if (quizStep === "1a") {
    questionPanel.innerHTML = `
      <div class="quiz-question-wrapper">
        ...
        <div class="quiz-question-card">
          What type of decision boundary was the model with only X₁ and X₂ (linear characteristics) able to draw?
        </div>
      </div>
      <div class="quiz-options-container">
        <button class="quiz-option-btn" data-correct="true" data-index="0">
          <span class="quiz-option-checkbox"></span>
          <span class="quiz-option-text">Linear (une separating straight line)</span>
        </button>
        ...
        <button class="quiz-option-btn" data-correct="false" data-index="2">
          <span class="quiz-option-checkbox"></span>
          <span class="quiz-option-text">No borders possible</span>
        </button>
      </div>
    `;
    ...
    questionPanel.innerHTML = `
      <div class="quiz-question-wrapper">
        <div class="quiz-question-badge">Question 1/2</div>
        <div class="quiz-question-card">
          What type of decision boundary was the model with X₁, X₂, X₁² and X₂² (quadratic characteristics) able to draw?
        </div>
      </div>
      <div class="quiz-options-container">
    ...
    questionPanel.innerHTML = `
      <div style="text-align: center; padding: 40px 20px; border: 1.5px dashed rgba(255,255,255,0.15); border-radius: 16px;">
        ...
        <p style="font-size: 13px; color: #94a3b8; line-height: 1.5;">
          Answer the open question to complete the exercise. You can move the box if it's blocking the playground.
        </p>
      </div>
    `;
    showQuiz2Overlay();
    ...
    btn.addEventListener("click", () => {
      ...
      if (quizStep === "1a") {
        feedbackText = isCorrect
          ...
          : "Not quite. X₁ and X₂ are linear Cartesian coordinates. Without hidden layers or transformations, the boundary remains rectilinear.";
      } else if (quizStep === "1b") {
        feedbackText = isCorrect
          ? "Exactly! The introduction of X₁² and X₂² allows the model to express quadratic terms, thus drawing circles or ellipses to separate the dataset."
          : "Remember the mathematical equations of a circle (x² + y² = r²). Quadratic characteristics are essential for drawing curved boundaries."
      }
...
function showQuiz2Overlay() {
  ...
  container.innerHTML = `
    <div class="dragdrop-header" style="cursor: move; display: flex; align-items: center; gap: 10px; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid rgba(255,255,255,0.1);">
      ...
      <h3 style="margin: 0; font-size: 16px; font-weight: 800; color: #fff;">Last Check! (Movable ↕️)</h3>
    </div>
    ...
    <div class="quiz-options-container" style="display: flex; flex-direction: column; gap: 10px;">
      ...
      <button class="quiz-option-btn opt-q2" data-correct="false">
        <span class="quiz-option-checkbox"></span>
        <span class="quiz-option-text">C) A characteristic that produces random and irregular outputs.</span>
      </button>
      <button class="quiz-option-btn opt-q2" data-correct="false">
        <span class="quiz-option-checkbox"></span>
  ...
  optButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (isCorrect) {
        ...
        q2Feedback.innerHTML = "<strong>Congratulations !</strong>You have identified that a non-linear transformation (e.g., x²) allows the data to be projected into a space where it becomes linearly separable.";
        ...
        q2Feedback.innerHTML = "<strong>Incorrect.</strong> Consider the transition from X to X²: we apply a non-linear function (the power of 2) to change the characteristic space.";
      }
    });
  });
}
  ...
  overlay.innerHTML = `
    <div class="completion-card">
      ...
      <p class="completion-msg">You have successfully completed exercise 3.<br>
        The analysis of linear and nonlinear boundaries is now complete.<br>
