...
// --- LOGIQUE DE SAUVEGARDE ET VALIDATION ---
      ...
      window.addEventListener('message', (event) => {
        if (event.data.type === 'EXO_SUCCESS' && (event.data.exoId == 11 || event.data.exoId == "11")) {
      ...
      btnSauvegarder.onclick = () => {
        ...
        const saved = saveToStorage('saved_exercises', { id: 11, date: now });
        if (saved) {
          btnSauvegarder.innerHTML = '✅ Sauvegardé !';
          btnSauvegarder.style.opacity = '0.7';
          btnSauvegarder.disabled = true;
        } else { alert("Cet exercice est déjà dans votre profil."); }
      };
      ...
      btnRealise.onclick = () => {
        ...
        if (!isLoggedIn) { window.location.href = 'Page-demo/register.html'; return; }
        const now = new Date().toLocaleDateString('fr-FR');
        const saved = saveToStorage('completed_exercises', { id: 11, date: now });
        if (saved) {
