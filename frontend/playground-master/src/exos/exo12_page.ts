...
btnSauvegarder.onclick = async () => {
    if ((window as any).StorageService) {
        const success = await (window as any).StorageService.save(12);
        if (success) {
...
function injectInfoTipsInIframe() {
    try {
        ...
        const testLossEl = iframeDoc.querySelector('.ui-testLoss');
        const trainLossEl = iframeDoc.querySelector('.ui-trainLoss');

        if (testLossEl && !testLossEl.querySelector('.info-tip-loss')) {
            const labelEl = testLossEl.querySelector('span');
            if (labelEl) {
                const tip = iframeDoc.createElement('span');
        ...
        if (trainLossEl && !trainLossEl.querySelector('.info-tip-loss')) {
            const labelEl = trainLossEl.querySelector('span');
            if (labelEl) {
                const tip = iframeDoc.createElement('span');
                tip.className = 'info-tip-loss info-tip-flash-active';
                tip.innerText = '?';
...
function renderActivity1() {
    ...
    const shuffledConcepts = [...matchingData.concepts].sort(() => Math.random() - 0.5);
    shuffledConcepts.forEach(c => {
        ...
        const zone = wrapper.querySelector(`#zone-${c.id}`) as HTMLElement;
...
function handleDropMatch(sourceId, conceptId) {
    ...
    const zone = document.getElementById(`zone-${conceptId}`);
    const wrapper = document.getElementById(`target-wrapper-${conceptId}`);

    if (!dragCard || !zone || !wrapper) return;
    ...
    if (sourceId === conceptId) {
        ...
        fPanel.innerHTML = `
            <div class="feedback-box" style="border-left-color: #10b981; background: rgba(16, 185, 129, 0.1);">
                ✅ Correct! You matched <strong>${matchingData.concepts.find(c => c.id === conceptId).name}</strong> with its definition.
            </div>
        `;
        ...
        fPanel.innerHTML = `
            <div class="feedback-box" style="border-left-color: #ef4444; background: rgba(239, 68, 68, 0.1);">
                ❌ Not quite. That definition doesn't fit the concept of <strong>${matchingData.concepts.find(c => c.id === conceptId).name}</strong>. Try again!
            </div>
        `;
    }
}
