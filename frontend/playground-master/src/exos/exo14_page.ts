...
const backgroundContainer = document.getElementById('background-container');
            ...
            window.addEventListener('message', (event) => {
                ...
                if (event.data.type === 'EXO_SUCCESS' && (event.data.exoId == 14 || event.data.exoId == "14")) {
                    console.log("Validation confirmée pour l'exercice 14 !");
            ...
            btnSauvegarder.onclick = () => {
                ...
                const saved = saveToStorage('saved_exercises', { id: 14, date: now });
                if (saved) {
            ...
            btnRealise.onclick = () => {
                ...
                if (!isLoggedIn) { window.location.href = 'Page-demo/register.html'; return; }
                const now = new Date().toLocaleDateString('fr-FR');
                const saved = saveToStorage('completed_exercises', { id: 14, date: now });
                if (saved) {
