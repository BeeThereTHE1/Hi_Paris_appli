...
btnRealise.onclick = function () {
    ...
    if (window.StorageService) {
        window.StorageService.complete(16).then(function (success) {
            if (success) {
...
function startTutorial() {
    ...
    var timeLeft = 8; // 8 seconds reading timer
    ...
    qPanel.innerHTML = `
        ...
        <div id="def-boxes-container">
            ...
            <div class="def-box color-2" id="def-box-2">
                ...
                • The gradient tells how to change each weight to reduce this error (en rouge)</p>
            </div>
    ...
    simulationCheckInterval = setInterval(function () {
        try {
            ...
            if (iframeSigmoid && iframeSigmoid.contentWindow) {
                ...
                var playBtnSig = docSig.getElementById('play-pause-button');
                sigmoidPlaying = playBtnSig && playBtnSig.classList.contains('playing') || false;
                if (sigmoidPlaying && playBtnSig) {
                    playBtnSig.classList.remove('play-btn-flash');
                }
            }
            ...
            if (iframeRelu && iframeRelu.contentWindow) {
                ...
                var playBtnRelu = docRelu.getElementById('play-pause-button');
                reluPlaying = playBtnRelu && playBtnRelu.classList.contains('playing') || false;
                if (reluPlaying && playBtnRelu) {
                    playBtnRelu.classList.remove('play-btn-flash');
                }
            }
