...
window.addEventListener('message', function (event) {
    if (event.data.type === 'EXO_SUCCESS' && event.data.exoId == 17) {
...
btnSauvegarder.onclick = function () {
    ...
    var saved = saveToStorage('saved_exercises', { id: 17, date: now });
    if (saved) {
...
btnRealise.onclick = function () {
    ...
    var now = new Date().toLocaleDateString('fr-FR');
    var saved = saveToStorage('completed_exercises', { id: 17, date: now });
    if (saved) {
