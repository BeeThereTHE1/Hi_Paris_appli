...
const backgroundContainer = document.getElementById('background-container');
      ...
      btnSauvegarder.onclick = () => {
        ...
        const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
        if (!isLoggedIn) {
          // Si l'utilisateur n'est pas connecté, redirection vers register.html
          window.location.href = 'Page-demo/register.html';
          return; // Empêche l'exécution du reste du code
        }
        // ---Test ---
        const now = new Date().toLocaleDateString('fr-FR');
        const saved = saveToStorage('saved_exercises', { id: 1, date: now });
        if (saved) {
      ...
      btnRealise.onclick = () => {
        ...
        const now = new Date().toLocaleDateString('fr-FR');
        // --- Tset--
        const saved = saveToStorage('completed_exercises', { id: 1, date: now });
        if (saved) {
          btnRealise.innerHTML = '✨ Validé !';
          btnRealise.disabled = true;
          // On redirige vers l'historique après un court délai pour que l'utilisateur voit le changement
