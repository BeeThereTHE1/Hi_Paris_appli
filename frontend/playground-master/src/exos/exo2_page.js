(function () {
  ...
  class Exo2 extends ExoPageBase {
    ...
    async init() {
      ...
      window.addEventListener('beforeunload', () => this.cleanupActiveListeners());
    }
    ...
    showHint(label, selector, title, text, compactTitleOnly) {
      ...
      box.style.left = (rect.left - 15 + window.scrollX) + 'px';
      box.style.top = (rect.top - 15 + window.scrollY) + 'px';
      box.style.width = (rect.width + 30) + 'px';
      box.style.height = (rect.height + 30) + 'px';
      document.body.appendChild(box);

      var dot = document.createElement('div');
      dot.className = 'tutorial-indicator-dot';
      dot.innerText = label;
      dot.style.left = (rect.left - 15 + window.scrollX) + 'px';
      dot.style.top = (rect.top - 15 + window.scrollY) + 'px';
