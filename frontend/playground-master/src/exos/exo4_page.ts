...
(function () {
      ...
      window.addEventListener('message', (event) => {
        if (event.data.type === 'EXO_SUCCESS' && event.data.exoId == 4) {
      ...
      btnSauvegarder.onclick = () => {
        ...
        const saved = saveToStorage('saved_exercises', { id: 4, date: now });
        if (saved) {
      ...
      btnRealise.onclick = () => {
        ...
        if (!isLoggedIn) { window.location.href = 'Page-demo/register.html'; return; }
        const now = new Date().toLocaleDateString('fr-FR');
        const saved = saveToStorage('completed_exercises', { id: 4, date: now });
        if (saved) {
